...
Route Master Data Kelaikan Stage 1 dan Stage 2 sudah memakai CRUD API aktif. Route `/fitness/*` non-master masih menampilkan placeholder tanpa API request, tanpa tombol simpan, dan tanpa mutation aktif. Setiap placeholder wajib menampilkan tujuan menu, field form, validasi ringkas, menu pemakai, dan hubungan ke Surveyor lapangan. Route placeholder khusus tahap ini mencakup `/fitness/applications/create` untuk Buat Permohonan dan `/fitness/containers/import` untuk Import Data Peti Kemas.
