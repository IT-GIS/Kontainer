...
function Damage({ rows }: { rows: Array<Record<string, unknown>> }) {
  return <DataTable rows={rows} columns={[{ key: "damage_no", header: "Damage No", render: (row) => String(row.damage_no ?? "-") }, { key: "location", header: "Location", render: (row) => `${row.face ?? "-"} ${row.internal_location ?? ""}` }, { key: "component", header: "Component", render: (row) => String(row.component_name ?? row.component_code ?? "-") }, { key: "damage", header: "Damage", render: (row) => String(row.damage_name ?? row.damage_code ?? "-") }, { key: "material", header: "Material", render: (row) => String(row.material_name ?? row.material_code ?? "-") }, { key: "responsibility", header: "Responsibility", render: (row) => String(row.responsibility_name ?? row.responsibility_code ?? "-") }, { key: "severity", header: "Severity", render: (row) => String(row.severity ?? "-") }, { key: "photo", header: "Photos", render: (row) => String(row.photo_count ?? 0) }]} />;
}
