...
export function SurveyorSurveyList({ title, description, fixedStatus = "", history = false }: Props) {
  ...
  return <div className="page-stack">
    ...
    <div className="toolbar">
      ...
      {history ? <label><span className="sr-only">Filter status pekerjaan survei</span><select value={status} onChange={(event) => { setPage(1); setStatus(event.target.value); }}><option value="">Semua Status</option>{["draft", "submitted", "under_review", "need_revision", "resubmitted", "approved", "rejected"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></label> : null}
    </div>
