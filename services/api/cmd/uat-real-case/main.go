...
func schemaGate(ctx context.Context, db *sql.DB) error {
	...
	for _, marker := range required {
		...
		if count != 1 {
			return fmt.Errorf("schema gate failed: missing %s.%s; apply complete migrations 0013-0016", marker[0], marker[1])
		}
	}
	return nil
}
...
func bootstrap(ctx context.Context, db *sql.DB, opt options) error {
	...
	payload, _ := json.Marshal(map[string]any{
		...
		"object_prefix": "uat/" + datasetID, "bootstrapped_users": len(users),
	})
	if _, err := tx.ExecContext(ctx, `
	...
	fmt.Printf("PASS schema_gate migrations=0013-0016\n")
	fmt.Printf("PASS users_bootstrapped count=%d password=REDACTED reviewer_role=supervisor customer_pic_role=none\n", len(users))
...
func verify(ctx context.Context, db *sql.DB) ([]check, error) {
	if err := schemaGate(ctx, db); err != nil {
		return nil, err
	}
	...
	checks := []queryCheck{
		...
		{"06 damage counter", `SELECT COUNT(*) FROM survey_damage_counters c WHERE c.last_number<>(SELECT COALESCE(MAX(CAST(SUBSTRING(sd.damage_no,3) AS UNSIGNED)),0) FROM survey_damages sd WHERE sd.survey_id=c.survey_id AND sd.deleted_at IS NULL)`, "consistent", "mismatch"},
		...
		{"13 revision history", `SELECT CASE WHEN EXISTS(SELECT 1 FROM job_orders WHERE job_order_no='UAT-JOB-2026-0805-003') AND NOT EXISTS(SELECT 1 FROM survey_revisions r JOIN surveys s ON s.id=r.survey_id JOIN job_orders j ON j.id=s.job_order_id WHERE j.job_order_no='UAT-JOB-2026-0805-003') THEN 1 ELSE 0 END`, "present/not finalized", "missing"},
