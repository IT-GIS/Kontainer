...
-- Management may inspect monitoring/review/report surfaces, but every mutation
-- remains denied even if an older baseline accidentally granted it.
...
-- Technical survey decisions belong to Supervisor/Super Admin. Admin keeps
-- reviews.view.all for operational monitoring only.
DELETE role_permission
FROM role_permissions role_permission
JOIN roles role ON role.id=role_permission.role_id
JOIN permissions permission ON permission.id=role_permission.permission_id
